...
class BooleanNetwork:
    ...
    def pinning_control_driver_nodes(self):
        ...
        to ``A``. Operationally, the test is: the pinning-controlled
        STG (``pcstg``) for each attractor must be a single weakly-
        connected component.
        ...
        2. *Sufficiency check*: build pcstg per attractor; require
           ``len(weakly_connected_components(pcstg)) == 1``.
        ...
        for n_pin in range(lower_bound, self.Nnodes):
            ...
            for pvs in itertools.combinations(nodeids, n_pin):
                ...
                for _att, pcstg in pcstg_dict.items():
                    if sum(1 for _ in nx.weakly_connected_components(pcstg)) > 1:
                        controlled = False
                        break
                if controlled:
                    result.append(pvs)
